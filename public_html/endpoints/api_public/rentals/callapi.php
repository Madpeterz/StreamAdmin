            ...
            if($rental->get_expireunixtime() > time())
            {
                ...
                if($stream->load($rental->get_streamlink()) == true)
                {
                    ...
                    if($server->load($stream->get_serverlink()) == true)
                    {
                        ...
                        else { $why_failed = "pending api calls - please wait and try again"; }
                    }
                    else { $why_failed = "Unable to load server attached to stream"; }
                }
                else { $why_failed = "Unable to load stream attached to rental"; }
            }
            else { $why_failed = "rental is expired - api disabled"; }
        }
        else { $why_failed = "rental ownership issue"; }
    }
    else { $why_failed = "Unable to load rental"; }
}
else { $why_failed = "unknown public api call made"; }
if($status == false)
{
    echo $why_failed;
}
