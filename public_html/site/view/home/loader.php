...
foreach($server_set->get_all_ids() as $server_id)
{
    $server = $server_set->get_object_by_id($server_id);
    $entry = array();
    $entry[] = '<a href="[[url_base]]stream/onserver/'.$server->get_id().'"><strong>'.$server->get_domain().'</strong></a><br/><br/><div class="serverstatusdisplay">
    <sub data-loading="
    CPU: <span class=\'text-light\'>?</span> |
    Ram: <span class=\'text-light\'>?</span> % |
    Str: <span class=\'text-light\'>?</span> %"
    data-repeatingrate="7000" class="ajaxonpageload" data-loadurl="[[url_base]]ajax.php/server/server_load/'.$server->get_id().'"></sub></div>';
