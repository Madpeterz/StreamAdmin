...
class SecondlifeApiMailserver extends TestCase
{
    public function test_Next()
    {
        ...
        $testsystem->forceProcessURI("Mailserver/Next");
        $_POST["mode"] = "test";
...
$storage = [
        ...
        $_POST["unixtime"] = time();
        $raw = time()  . "MailserverNext". implode("",$real) . $testsystem->getSlConfig()->getSlLinkCode();
        $_POST["hash"] = sha1($raw);
        ...
        $messageSet = new MessageSet();
        $this->assertSame(true,$messageSet->loadAll()->status,"Unable to load message set to check workspace");
        $this->assertSame(3,$messageSet->getCount(),"Incorrect number of messages in the Q");
        $botmessageQ = new BotcommandqSet();
        $botmessageQ->loadAll();
