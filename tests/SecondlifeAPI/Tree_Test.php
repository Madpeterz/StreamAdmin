...
class SecondlifeApiTree extends TestCase
{
    public function test_RecreateTreeVender()
    {
        ...
        $package = new Package();
        $this->assertSame(true,$package->loadByName("AlsoUnitTestPackage")->status,"Load package failed");
    ...
    public function test_Getpackages()
    {
        ...
        $testsystem->forceProcessURI("Tree/Getpackages");
        $_POST["method"] = "Tree";
...
$storage = [
        ...
        $_POST["unixtime"] = time();
        $raw = time()  . "TreeGetpackages" . implode("",$real) . $testsystem->getSlConfig()->getSlLinkCode();
        $_POST["hash"] = sha1($raw);
        $treevender = new Treevender();
        $this->assertSame(true,$treevender->loadByName("Demo")->status,"Load ok failed");
