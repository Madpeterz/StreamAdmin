...
class DefaultView extends View
{
    ...
    public function process(): void
    {
        ...
        $form->select("limitStreams", "Enforce total streams", $slConfigObj->getLimitStreams(), $this->yesNo);
        $form->select("limitTime", "Enforce rental time limit", $slConfigObj->getLimitTime(), $this->yesNo);
        $form->numberInput(
            "maxStreamTimeDays",
            "Max rental in days (7 to 365)",
            $slConfigObj->getMaxStreamTimeDays(),
            365,
            "Max days"
        );
        $form->numberInput(
            "maxTotalStreams",
            "Max total streams (1 to 2000)",
            $slConfigObj->getMaxTotalStreams(),
            2000,
            "Max number of streams per client"
        );
