<?php

namespace App\R7\Model;

use YAPF\DbObjects\GenClass\GenClass as GenClass;

// Do not edit this file, rerun gen.php to update!
class Rental extends genClass
{
    protected $use_table = "rental";
    // Data Design
    protected $fields = [
        "id",
        "avatarLink",
        "streamLink",
        "packageLink",
        "noticeLink",
        "startUnixtime",
        "expireUnixtime",
        "renewals",
        "totalAmount",
        "message",
        "rentalUid",
        "apiAllowAutoSuspend",
        "apiSuspended",
        "apiPendingAutoSuspend",
        "apiPendingAutoSuspendAfter",
    ];
    protected $dataset = [
        "id" => ["type" => "int", "value" => null],
        "avatarLink" => ["type" => "int", "value" => null],
        "streamLink" => ["type" => "int", "value" => null],
        "packageLink" => ["type" => "int", "value" => null],
        "noticeLink" => ["type" => "int", "value" => null],
        "startUnixtime" => ["type" => "int", "value" => null],
        "expireUnixtime" => ["type" => "int", "value" => null],
        "renewals" => ["type" => "int", "value" => 0],
        "totalAmount" => ["type" => "int", "value" => 0],
        "message" => ["type" => "str", "value" => null],
        "rentalUid" => ["type" => "str", "value" => null],
        "apiAllowAutoSuspend" => ["type" => "bool", "value" => 1],
        "apiSuspended" => ["type" => "bool", "value" => 0],
        "apiPendingAutoSuspend" => ["type" => "bool", "value" => 0],
        "apiPendingAutoSuspendAfter" => ["type" => "int", "value" => null],
    ];
    // Getters
    public function getAvatarLink(): ?int
    {
        return $this->getField("avatarLink");
    }
    public function getStreamLink(): ?int
    {
        return $this->getField("streamLink");
    }
    public function getPackageLink(): ?int
    {
        return $this->getField("packageLink");
    }
    public function getNoticeLink(): ?int
    {
        return $this->getField("noticeLink");
    }
    public function getStartUnixtime(): ?int
    {
        return $this->getField("startUnixtime");
    }
    public function getExpireUnixtime(): ?int
    {
        return $this->getField("expireUnixtime");
    }
    public function getRenewals(): ?int
    {
        return $this->getField("renewals");
    }
    public function getTotalAmount(): ?int
    {
        return $this->getField("totalAmount");
    }
    public function getMessage(): ?string
    {
        return $this->getField("message");
    }
    public function getRentalUid(): ?string
    {
        return $this->getField("rentalUid");
    }
    public function getApiAllowAutoSuspend(): ?bool
    {
        return $this->getField("apiAllowAutoSuspend");
    }
    public function getApiSuspended(): ?bool
    {
        return $this->getField("apiSuspended");
    }
    public function getApiPendingAutoSuspend(): ?bool
    {
        return $this->getField("apiPendingAutoSuspend");
    }
    public function getApiPendingAutoSuspendAfter(): ?int
    {
        return $this->getField("apiPendingAutoSuspendAfter");
    }
    // Setters
    /**
    * setAvatarLink
    * @return mixed[] [status =>  bool, message =>  string]
    */
    public function setAvatarLink(?int $newvalue): array
    {
        return $this->updateField("avatarLink", $newvalue);
    }
    /**
    * setStreamLink
    * @return mixed[] [status =>  bool, message =>  string]
    */
    public function setStreamLink(?int $newvalue): array
    {
        return $this->updateField("streamLink", $newvalue);
    }
    /**
    * setPackageLink
    * @return mixed[] [status =>  bool, message =>  string]
    */
    public function setPackageLink(?int $newvalue): array
    {
        return $this->updateField("packageLink", $newvalue);
    }
    /**
    * setNoticeLink
    * @return mixed[] [status =>  bool, message =>  string]
    */
    public function setNoticeLink(?int $newvalue): array
    {
        return $this->updateField("noticeLink", $newvalue);
    }
    /**
    * setStartUnixtime
    * @return mixed[] [status =>  bool, message =>  string]
    */
    public function setStartUnixtime(?int $newvalue): array
    {
        return $this->updateField("startUnixtime", $newvalue);
    }
    /**
    * setExpireUnixtime
    * @return mixed[] [status =>  bool, message =>  string]
    */
    public function setExpireUnixtime(?int $newvalue): array
    {
        return $this->updateField("expireUnixtime", $newvalue);
    }
    /**
    * setRenewals
    * @return mixed[] [status =>  bool, message =>  string]
    */
    public function setRenewals(?int $newvalue): array
    {
        return $this->updateField("renewals", $newvalue);
    }
    /**
    * setTotalAmount
    * @return mixed[] [status =>  bool, message =>  string]
    */
    public function setTotalAmount(?int $newvalue): array
    {
        return $this->updateField("totalAmount", $newvalue);
    }
    /**
    * setMessage
    * @return mixed[] [status =>  bool, message =>  string]
    */
    public function setMessage(?string $newvalue): array
    {
        return $this->updateField("message", $newvalue);
    }
    /**
    * setRentalUid
    * @return mixed[] [status =>  bool, message =>  string]
    */
    public function setRentalUid(?string $newvalue): array
    {
        return $this->updateField("rentalUid", $newvalue);
    }
    /**
    * setApiAllowAutoSuspend
    * @return mixed[] [status =>  bool, message =>  string]
    */
    public function setApiAllowAutoSuspend(?bool $newvalue): array
    {
        return $this->updateField("apiAllowAutoSuspend", $newvalue);
    }
    /**
    * setApiSuspended
    * @return mixed[] [status =>  bool, message =>  string]
    */
    public function setApiSuspended(?bool $newvalue): array
    {
        return $this->updateField("apiSuspended", $newvalue);
    }
    /**
    * setApiPendingAutoSuspend
    * @return mixed[] [status =>  bool, message =>  string]
    */
    public function setApiPendingAutoSuspend(?bool $newvalue): array
    {
        return $this->updateField("apiPendingAutoSuspend", $newvalue);
    }
    /**
    * setApiPendingAutoSuspendAfter
    * @return mixed[] [status =>  bool, message =>  string]
    */
    public function setApiPendingAutoSuspendAfter(?int $newvalue): array
    {
        return $this->updateField("apiPendingAutoSuspendAfter", $newvalue);
    }
    // Loaders
    public function loadByAvatarLink(int $avatarLink): bool
    {
        return $this->loadByField("avatarLink", $avatarLink);
    }
    public function loadByStreamLink(int $streamLink): bool
    {
        return $this->loadByField("streamLink", $streamLink);
    }
    public function loadByPackageLink(int $packageLink): bool
    {
        return $this->loadByField("packageLink", $packageLink);
    }
    public function loadByNoticeLink(int $noticeLink): bool
    {
        return $this->loadByField("noticeLink", $noticeLink);
    }
    public function loadByStartUnixtime(int $startUnixtime): bool
    {
        return $this->loadByField("startUnixtime", $startUnixtime);
    }
    public function loadByExpireUnixtime(int $expireUnixtime): bool
    {
        return $this->loadByField("expireUnixtime", $expireUnixtime);
    }
    public function loadByRenewals(int $renewals): bool
    {
        return $this->loadByField("renewals", $renewals);
    }
    public function loadByTotalAmount(int $totalAmount): bool
    {
        return $this->loadByField("totalAmount", $totalAmount);
    }
    public function loadByMessage(string $message): bool
    {
        return $this->loadByField("message", $message);
    }
    public function loadByRentalUid(string $rentalUid): bool
    {
        return $this->loadByField("rentalUid", $rentalUid);
    }
    public function loadByApiAllowAutoSuspend(bool $apiAllowAutoSuspend): bool
    {
        return $this->loadByField("apiAllowAutoSuspend", $apiAllowAutoSuspend);
    }
    public function loadByApiSuspended(bool $apiSuspended): bool
    {
        return $this->loadByField("apiSuspended", $apiSuspended);
    }
    public function loadByApiPendingAutoSuspend(bool $apiPendingAutoSuspend): bool
    {
        return $this->loadByField("apiPendingAutoSuspend", $apiPendingAutoSuspend);
    }
    public function loadByApiPendingAutoSuspendAfter(int $apiPendingAutoSuspendAfter): bool
    {
        return $this->loadByField("apiPendingAutoSuspendAfter", $apiPendingAutoSuspendAfter);
    }
}
// please do not edit this file
