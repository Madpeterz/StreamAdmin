    ...
    public function process(): void
    {
        ...
        if ($notecards == "none") {
            if ($notice_notecard_set->getCount() > 0) {
                ...
                if ($status["status"] == false) {
                    $this->output->setSwapTagString("message", "Unable to mark static notecards as missing");
                    return;
                }
            }
            $this->output->setSwapTagString("status", "true");
            $this->output->setSwapTagString("message", "Old notecards removed");
            return;
        }

        if (strlen($notecards) == 0) {
            $this->output->setSwapTagString("message", "Expected notecards list not sent!");
            return;
        }
        ...
        if ($status == true) {
            foreach ($notecards_list as $notecardname) {
                ...
                if ($status == false) {
                    $this->output->setSwapTagString("message", "Unable to create static notecard entry");
                    return;
                }
            ...
            foreach ($notice_notecard_set->getAllIds() as $notice_notecard_id) {
                if (in_array($notice_notecard_id, $used_notecards) == false) {
                    ...
                    if ($notice_notecard->getMissing() == true) {
                        ...
                        if ($status == false) {
                            $this->output->setSwapTagString("message", "Unable to remove static notecard entry");
                            return;
                        }
                    }
                }
