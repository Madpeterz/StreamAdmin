...
class Startrental extends SecondlifeAjax
{
    ...
    public function process(): void
    {
        ...
        if ($package == null) { // find package
            $this->output->setSwapTagString("message", "Unable to find");
            return;
        }

        $avatar = $this->getAvatar($input->postFilter("avataruuid"), $input->postFilter("avatarname"));
        if ($avatar == null) {
            $this->output->setSwapTagString("message", "Unable to attach avatar");
            return;
        }
        ...
        if ($stream == null) {
            $this->output->setSwapTagString("message", "Unable to find a unsold stream in that package");
            return;
        }
        ...
        if (array_key_exists($amountpaid, $accepted_payment_amounts) == false) {
            $this->output->setSwapTagString("message", "Payment amount not accepted");
            return;
        }
        ...
        if ($status == false) {
            $this->output->setSwapTagString("message", "Unable to create rental uid");
            return;
        }
        ...
        if ($status == false) {
            $this->output->setSwapTagString("message", "Unable to create rental");
            return;
        }
        ...
        if ($status == false) {
            $this->output->setSwapTagString("message", "Unable to update rental link for stream");
            return;
        }

        $status = $this->createTransaction($avatar, $package, $stream, $this->reseller, $this->region, $amountpaid);
        if ($status == false) {
            $this->output->setSwapTagString("message", "Unable to create transaction");
            return;
        }

        $this->output->setSwapTagString("owner_payment", 0);
        if ($this->owner_override == false) {
            $avatar_system = new Avatar();
            if ($avatar_system->loadID($this->slconfig->getOwner_av()) == false) {
                $this->output->setSwapTagString("message", "Unable to load owner avatar");
                return;
            }
            ...
            $this->output->setSwapTagString("owner_payment", 1);
            $this->output->setSwapTagString("owner_payment_amount", $left_over);
            $this->output->setSwapTagString("owner_payment_uuid", $avatar_system->getAvataruuid());
        }
        ...
        $this->output->setSwapTagString("status", $status);
    }
}
