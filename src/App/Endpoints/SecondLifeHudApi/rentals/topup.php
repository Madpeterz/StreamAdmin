                    ...
                    if ($amount == $package->getCost()) {
                        ...
                        if ($tidhashcheck == $tidhash) {
                            ...
                            if ($avatar_system->loadID($slconfig->get_owner_av()) == true) {
                                ...
                                $why_failed = "cant load system owner";
                            }
                        } else {
                            $why_failed = "tid hash error";
                        }
                    } else {
                        $why_failed = "incorrect amount";
                    }
                } else {
                    $why_failed = "cant find package";
                }
            } else {
                $why_failed = "ownership issue for topup";
            }
        } else {
            $why_failed = "Cant find rental";
        }
    } else {
        $why_failed = "Unknown region";
    }
} else {
    $why_failed = "One or more values are null";
}
