...
if (strlen($message) >= 10) {
    if (strlen($message) <= 800) {
        ...
        if ($ok == true) {
            $this->output->addSwapTagString("page_title", " Bulk sending to " . $page . ": " . $souce_named . "");
            $stream_set = new stream_set();
            ...
            if ($max_avatar_count > 0) {
                ...
                $this->output->addSwapTagString("page_content", "<br/><hr/>Note: If an avatar has multiple streams that match the selected filter source the first rental will be used.");
            } else {
                $this->output->redirect("outbox?message=No selectable avatars for the " . $page . "");
            }
        } else {
