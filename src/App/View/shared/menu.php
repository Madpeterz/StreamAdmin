<?php
    ...
    foreach ($menu_items as $menu_key => $menu_config) {
        ...
        if (in_array($module, $menu_config["active_on"]) == true) {
            $output .= " active";
            $this->output->addSwapTagString("page_breadcrumb_icon", '<i class="' . $menu_config["icon"] . ' text-success"></i>');
            $this->output->addSwapTagString("page_breadcrumb_text", '<a href="[[url_base]]' . $menu_config["target"] . '">' . $menu_key . '</a>');
        }
