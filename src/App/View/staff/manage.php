...
if ($session->get_ownerlevel() == true) {
    ...
    $this->output->setSwapTagString("page_actions", "<a href='[[url_base]]staff/remove/" . $this->page . "'><button type='button' class='btn btn-danger'>Remove</button></a>");

    $staff = new staff();
    if ($staff->loadByField("id", $this->page) == true) {
        $form = new form();
