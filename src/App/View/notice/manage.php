...
$this->output->setSwapTagString("page_actions", "<a href='[[url_base]]notice/remove/" . $page . "'><button type='button' class='btn btn-danger'>Remove</button></a>");
$where_config = array(
...
if ($notice->load($page) == true) {
    if ($notice->get_hoursremaining() < 999) {
        ...
        $this->output->addSwapTagString("page_title", ":" . $notice->get_name());
        $form = new form();
        $form->target("notice/update/" . $page . "");
        $form->required(true);
        $form->col(6);
            $form->group("Basic");
            $form->textInput("name", "Name", 30, $notice->get_name(), "Name");
        ...
        $form->col(6);
            ...
            $form->numberInput("hoursremaining", "Hours remain [Trigger at]", $notice->get_hoursremaining(), 3, "Max value 998");
        $form->col(12);
            $form->directAdd("<br/>");
        $form->col(6);
        ...
        if (in_array($use_notecard_link, $notice_notecard_set->get_all_ids()) == false) {
            $use_notecard_link = 1;
            $form->directAdd("<div class=\"alert alert-danger\" role=\"alert\">Current notecard \"" . $current_notecard_notice->get_name() . "\" is missing</div>");
        }
