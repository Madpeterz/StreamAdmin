...
$this->output->setSwapTagString("page_actions", "<a href='[[url_base]]template/remove/" . $page . "'><button type='button' class='btn btn-danger'>Remove</button></a>");
$template = new template();
if ($template->load($page) == true) {
    $this->output->addSwapTagString("page_title", ":" . $template->get_name());
    $form = new form();
    $form->target("template/update/" . $page . "");
    $form->required(true);
    $form->col(3);
        $form->textInput("name", "Name", 30, $template->get_name(), "Name");
    $form->split();
    $form->col(6);
