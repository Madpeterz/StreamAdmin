...
{
    protected function bindParams($sql, $options = [], $optionWord = "WHERE", $optionJoin = "AND")
    {
        if(count($options) > 0)
        {
            $sql .= " " . $optionWord;
    ...
    protected function bindParamsMode2($sql, $options = [], $startWord = "", $splitter = "", $endWord = "")
    {
        if(count($options) > 0)
        {
            $sql .= " " . $startWord;
    ...
    protected function sql_bind(&$stmt, $values = [])
    {
        $args = [];
        if(count($values) > 0)
        {
