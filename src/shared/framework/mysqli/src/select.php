    ...
    public function selectV2(array $basic_config,?array $order_config=null,?array $where_config=null,?array $options_config=null,?array $join_tables=null) : array
    {
        if(array_key_exists("table",$basic_config) == true)
        {
            if($this->sqlStart() == true)
            {
                ...
                {
                    /*
                    "types" => [], // "LEFT JOIN", "JOIN", "RIGHT JOIN" ect
                    "tables" => [], // "tablename x"   example  people pl
                    "onfield_left" => [], // "pl.id"
                    "onfield_match" => [], // "=" "!=" ">" ect
                    "onfield_right" => [], // "mtb.id"
                    */
                    $failed = true;
                    $all_found = true;
                    $counts_match = true;
                ...
                if($failed == false)
                {
                    ...
                    else return array("status"=>true,"message"=>"no data empty in array.","dataSet"=>[]);
                }
