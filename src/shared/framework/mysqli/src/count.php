...
abstract class mysqli_count extends mysqli_select
{
        ...
        if($this->sqlStart() == true)
        {
            if(count($tables) > 1)
            {
                ...
                if($failed == false)
                {
                    ...
                    else return array("status"=>true,"dataset"=>[],"message"=>"No data empty IN array");
                }
     ...
     */
    public function count(string $table,array $wherefields = [],array $wherevalues = [],string $joinOption = "AND") : array
	{
     ...
     * @deprecated this function has been replaced by basic_count
     */
    public function countQuery(string $table,string $count_field="id",array $wherefields = [],array $wherevalues = [])
    {
