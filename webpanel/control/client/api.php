...
if($rental->load_by_field("rental_uid",$page) == true)
{
    ...
    if($stream->load($rental->get_streamlink()) == true)
    {
        ...
        if(method_exists($server_api_helper,$functionname) == true)
        {
            ...
            {
                if($status == true) $ajax_reply->set_swap_tag_string("message",sprintf($lang["client.api.passed"],$server_api_helper->get_message()));
                else $ajax_reply->set_swap_tag_string("message",sprintf($lang["client.api.failed"],$server_api_helper->get_message()));
            }
            else
            {
                if($status == true) $ajax_reply->set_swap_tag_string("message",sprintf($lang["client.api.passed"],"No message from api helper"));
                else $ajax_reply->set_swap_tag_string("message",sprintf($lang["client.api.failed"],"No message from api helper"));
            }
        }
        else
        {
