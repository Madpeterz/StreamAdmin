...
class MailServerEndpointTest extends TestWorker
{
    ...
    {
        $mail = new Message();
        $mail->setAvatarLink(1);
        $mail->setMessage("Test message");
        $result = $mail->createEntry();
        $this->assertSame(true, $result->status, "Failed to create message entry");
        $this->assertSame("ok", $result->message, "Failed to create message entry");

        global $system;
        $system->setModule("Mailserver");
        $system->setArea("Next");
        $this->slAPI();
        $Mailserver = new Next();
        $status = $Mailserver->getOutputObject()->addSwapTagString("message");
        $this->assertSame("ready", $status, "Mailserver should have 'ready' status before processing");
        $Mailserver->process();
        $reply = $Mailserver->getOutputObject();
        $this->assertTrue(method_exists($reply, 'getSwapTagString'), 'Output object should have getSwapTagString method');
        $this->assertSame("Test message", $reply->getSwapTagString("message"), "Expected output should be 'ok'");
        $this->assertSame(true, $reply->getSwapTagBool("status"), "incorrect status in reply");
        $this->assertSame(true, $reply->getSwapTagBool("hasmessage"), "Expected hasmessage to be true");
    }
