...
class DetailsEndpointTest extends TestWorker
{
    ...
    public function test_Startrental()
    {
        ...
        $detail->loadAll();
        $result = $detail->purgeCollection();
        $this->assertSame(true, $result->status, "Failed to purge detail collection");
    }
    /**
    ...
    {
        global $system;
        $system->setModule("Details");
        $system->setArea("Send");
        $this->slAPI();
        $rental = new Rental();
        $rental->loadId(1);
        $this->assertSame(true, $rental->isLoaded(), "Rental should be loaded successfully");
        $hasRental = new Send();
        ...
        $this->assertTrue(method_exists($reply, 'getSwapTagString'), 'Output object should have getSwapTagString method');
        $this->assertSame("Details request accepted, it should be with you shortly!", $reply->getSwapTagString("message"), "Expected output should be 'some'");
        $this->assertSame(true, $reply->getSwapTagBool("status"), "incorrect status in reply");
    }
