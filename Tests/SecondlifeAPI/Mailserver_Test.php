...
class SecondlifeApiMailserver extends Mytest
{
    public function test_Next()
    {
        ...
        $this->assertSame(2, $messageSet->getCount(), "Incorrect number of messages in the Q");
        ...
        $Next->process();
        $this->assertStringStartsWith("Web panel reset link", $Next->getOutputObject()->getSwapTagString("message"), "incorrect message loaded");
        ...
        $messageSet = new MessageSet();
        $this->assertSame(true, $messageSet->loadAll()->status, "Unable to load message set to check workspace");
        $this->assertSame(1, $messageSet->getCount(), "Incorrect number of messages in the Q");
        $botmessageQ = new BotcommandqSet();
        $botmessageQ->loadAll();
