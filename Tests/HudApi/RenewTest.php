...
use App\Endpoint\Hudapi\Rentals\Renew;
...
class RenewTest extends TestWorker
{
     ...
     */
    public function test_HudRenewRental()
    {
        ...
        $_POST["transactionUUID"] = "289c3e36-69b3-40c5-9229-0c6a5d230566";
        $_POST["amountpaid"] = 246;
        $_POST["avatarName"] = "Madpeter Zond";
        $_POST["avatarUUID"] = "289c3e36-69b3-40c5-9229-0c6a5d230766";
        $_POST["rentalUid"] = $rental->getRentalUid();

        $Renew = new Renew();
        $reply = $Renew->getOutputObject();
        $this->assertSame("ready", $reply->getSwapTagString("message"), "object is not ready raw=".$_POST["raw"]);
        $Renew->process();
        $reply = $Renew->getOutputObject();
        
        $this->assertStringContainsString("Payment accepted there is now", $reply->getSwapTagString("message"), "expected reply for message is not correct");
        $this->assertSame(true,$reply->getSwapTagBool("status"), "incorrect status code");
    }
}
