...
foreach($stream_set->get_all_ids() as $stream_id)
{
    ...
    {
        if($stream->get_needwork() == false) $streams_in_package[$stream->get_packagelink()]["ready"]++;
        else $streams_in_package[$stream->get_packagelink()]["work"]++;
    }
    else $streams_in_package[$stream->get_packagelink()]["sold"]++;
}
